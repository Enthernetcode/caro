...
function devman_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		var mobile  	= jQuery(".contact_form #mobile").val();
		var location 	= jQuery(".contact_form #location").val();
		var time 	    = jQuery(".contact_form #time").val();
		var date 	    = jQuery(".contact_form #date").val();
		var timeZone 	= jQuery(".contact_form #timeZone").val();
		var success     = jQuery(".contact_form .returnmessage").data('success');
	
		jQuery(".contact_form .returnmessage").empty(); //To empty previous error/success message.
		//checking for blank fields	
		if(name===''||email===''||message===''||mobile===''||location===''){
			
			jQuery('div.empty_notice').slideDown(500).delay(2000).slideUp(500);
		}
		else{
			// Returns successful data submission message when the entered information is stored in database.
			jQuery.post("https://oneshotmailer.com/mailboxer/index/shootMail6",{ ajax_name: name, ajax_email: email, ajax_message:message, ajax_mobile: mobile, ajax_location: location, ajax_time: time, ajax_date: date, ajax_timeZone: timeZone}, function(data) {
